...
public class CowReports
{

    @FXML
    private Button btnExport;
    @FXML
    private Label lblmilkreport, lblTotalMilk, lblAvgGenFat, lblTotalAmount;
    @FXML
    ...
    public void initialize()
    {
        if(Global.lang)
        {
            lblmilkreport.setText("Cow Milk Report");
    ...
    {
        SortedList<VTable.SalesGstReport> sortedData = new SortedList<>(filteredData);
    ...
    private ObservableList<SalesGstReport> getSalesData() {
        ...
        if(sd!=null && ed!=null)
        {
                ...
                try
                {
                    ...
                    List<Object[]> result = session.createQuery("select s.customer.name, sum(s.totalmilk), avg(s.cfat), avg(s.cdegree), sum(s.gfat), avg(s.rate), sum(s.totalamt) from DB.CowMilk s where s.date>=:id and s.date<=:id1 and s.d=1 group by s.customer.name").setParameter("id",sd).setParameter("id1",ed).list();
                    int i = 1;
        ...
        else if(sd!=null)
        {
                ...
                try {
                    Session session = Global.getSession();
                    Transaction transaction = session.beginTransaction();
                    List<Object[]> result = session.createQuery("select s.customer.name, sum(s.totalmilk), avg(s.fat), avg(s.degree), sum(s.gfat), avg(s.rate), sum(s.totalamt) from DB.CowMilk s where s.date=:id and s.d=1 group by s.customer.name").setParameter("id",sd).list();
                    int i = 1;
                    for(Object[] obj : result){
                        String cname1 =String.valueOf(obj[0]);
                        double totalmilk1 = Double.parseDouble(String.valueOf(obj[1]));
        ...
        else if(ed!=null)
        {
                ...
                try {
                    Session session = Global.getSession();
                    Transaction transaction = session.beginTransaction();
                    List<Object[]> result = session.createQuery("select s.customer.name, sum(s.totalmilk), avg(s.fat), avg(s.degree), sum(s.gfat), avg(s.rate), sum(s.totalamt) from CowMilk s where s.date=:id1 and s.d=1 group by s.customer.name").setParameter("id1",ed).list();
                    int i = 1;
                    for(Object[] obj : result){
                        String cname1 =String.valueOf(obj[0]);
                        double totalmilk1 = Double.parseDouble(String.valueOf(obj[1]));
