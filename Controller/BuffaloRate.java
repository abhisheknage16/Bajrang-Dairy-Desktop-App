...
public class BuffaloRate {
    ...
    public void initialize()
    {
        if(Global.lang)
        {
            lblchangerate.setText("Buffalo Milk Price Change");
        ...
        try{
            ...
            DB.Rate rate = (DB.Rate) session.createQuery("from DB.Rate s where s.name=:id and s.cur=:id1 and s.d=1").setParameter("id1",2).setParameter("id", "buffalo").uniqueResult();
        ...
        add1.setOnAction(event -> {
            if(!Global.isDouble(newrate1.getText()))
                if (!Global.lang)
                    warn1.setText("कृपया योग्य दर टाका..!!");
                else
